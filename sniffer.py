...
def send_to_flask(domain):
	try:
		r = requests.get("http://localhost:5000/dns?domain="+domain,timeout=0.1)
	except Exception as e:
		print("Report to flask error ",e)		
...
if __name__ == "__main__":
	...
	if options.exportPath:
		...
		try:
			from scapy.all import sniff	# start_capture++
			from scapy.all import ARP 	# Address Resolution Protocol
			from scapy.all import IP	# Internet Protocol
			from scapy.all import IPv6	# IPv6
			from scapy.all import UDP	# User Data Protocol
			from scapy.all import DNS	# Domain Name System
			from scapy.all import DNSQR	# Domain Name System Query
		except ImportError:
			from sys import exit
